...
```
archiveImage: export 'nixl-ci-<ucx_version>-<BUILD_NUMBER>' -> <path>
```
...
scctl client connect -- srun -p mizu --ntasks=1 \
  ...
  enroot create --name nixl-debug-<ucx_version>-<BUILD_NUMBER> \
  /enroot_images/nixl-debug-<ucx_version>-<arch>-<BUILD_NUMBER>.sqsh
```
...
scctl client connect -- srun -p mizu --ntasks=1 \
  env ENROOT_DATA_PATH=/enroot-data/enroot-data-148069/user-148069 \
  enroot create --name nixl-debug-v1.21.x-2209 \
  /enroot_images/nixl-debug-v1.21.x-x86_64-2209.sqsh
```
...
scctl client connect -- srun -p mizu --ntasks=1 --pty \
  env ENROOT_DATA_PATH=/enroot-data/enroot-data-148069/user-148069 \
  enroot start --rw nixl-debug-v1.21.x-2209
```
...
preserved in the exported sqsh. The Jenkins log prints their location before
archiving — look for `=== CI DEBUG: core dump search` in the failing step's output.
